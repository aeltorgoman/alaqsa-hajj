...
function Sidebar({ page, setPage, count }: { page: string; setPage: (p: string) => void; count: number }) {
  return (
      ...
      try {
        const res = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{ role: "user", content: [{ type: "image", source: { type: "base64", media_type: file.type, data: base64 } }, { type: "text", text: `استخرج بيانات جواز السفر وأجب فقط بـ JSON: {"name_en":"","name_ar":"","short_en":"","short_ar":"","passport":"","national_id":"الرقم الشخصي/القومي من الجواز القطري أو المصري","nationality":"","dob":"","expiry":"","gender":"ذكر أو أنثى"}` }] }]
          })
        });
...
function PassengersPage({ passengers, setPassengers }: { passengers: Passenger[]; setPassengers: (p: Passenger[]) => void }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
          ...
          {filtered.length === 0 ? <div style={{ textAlign: "center", padding: "2rem", color: "#aaa", fontSize: 12 }}>لا توجد نتائج</div> :
            filtered.map(p => (
              <div key={p.id} onClick={() => setSelected(p)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 10, marginBottom: 3, cursor: "pointer", background: selected?.id === p.id ? "#E1F5EE" : "transparent", border: `0.5px solid ${selected?.id === p.id ? "#5DCAA5" : "transparent"}` }}
                onMouseEnter={e => { if (selected?.id !== p.id) e.currentTarget.style.background = "#f9f9f9"; }}
      ...
      {selected && !editing && (
        <div style={{ width: 280, borderRight: "0.5px solid #e5e5e5", overflowY: "auto", padding: 14 }}>
          <div style={{ textAlign: "center", marginBottom: 14, background: "#f9f9f9", borderRadius: 10, padding: 12 }}>
            <Avatar name={selected.name_ar} gender={selected.gender} size={52} />
            <div style={{ fontSize: 14, fontWeight: 600, marginTop: 8 }}>{selected.name_ar}</div>
            <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>{selected.name_en}</div>
            <div style={{ display: "flex", gap: 4, justifyContent: "center", marginTop: 6, flexWrap: "wrap" }}>
              <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99, background: selected.gender === "أنثى" ? "#FBEAF0" : "#E6F1FB", color: selected.gender === "أنثى" ? "#72243E" : "#0C447C" }}>{selected.gender}</span>
              {selected.services?.bus === "VIP" && <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 99, background: "#FAEEDA", color: "#633806" }}>⭐ VIP</span>}
            </div>
          </div>
          {[["🛂 الجواز", selected.passport], ["🪪 الرقم الشخصي", selected.national_id], ["🌍 الجنسية", selected.nat], ["🎂 الميلاد", selected.dob], ["📅 انتهاء الجواز", selected.expiry], ["📞 التليفون", selected.phone]].filter(([, v]) => v).map(([label, val]) => (
            <div key={label as string} style={{ background: "#f9f9f9", borderRadius: 8, padding: "7px 10px", marginBottom: 5 }}>
              <div style={{ fontSize: 10, color: "#888" }}>{label as string}</div>
              <div style={{ fontSize: 12, fontWeight: 500 }}>{val as string}</div>
            </div>
          ))}
      ...
      {editing && (
        <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "white", borderRadius: 12, width: "90%", maxWidth: 460, maxHeight: "85%", overflowY: "auto", boxShadow: "0 8px 32px rgba(0,0,0,0.15)" }}>
            <div style={{ padding: "12px 16px", borderBottom: "0.5px solid #e5e5e5", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "white" }}>
              ...
              <button onClick={() => setEditing(null)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 18, color: "#888" }}>✕</button>
            </div>
                ...
                <div style={{ padding: "8px 12px", borderTop: `0.5px solid ${isVIP ? "#F5C842" : "#e5e5e5"}` }}>
                  {busPassengers.length ? busPassengers.map((p, i) => (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 4px", borderRadius: 6, marginBottom: 2 }}>
                      ...
                      {p.services?.bus === "VIP" && <span style={{ fontSize: 9, background: "#FAEEDA", color: "#633806", padding: "1px 5px", borderRadius: 99 }}>⭐ طلب VIP</span>}
...
function ReportsPage({ passengers }: { passengers: Passenger[] }) {
  ...
  return (
    <div style={{ padding: 14, overflowY: "auto", height: "100%" }}>
      {!activeReport ? (
        <>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 12 }}>اختر التقرير اللي عايز تشوفه</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {reports.map(r => (
              <div key={r.id} onClick={() => setActiveReport(r.id)} style={{ border: "0.5px solid #e5e5e5", borderRadius: 12, padding: "14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 4, marginTop: 5 }}>
                    ...
                    <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 99, background: "#f0f0f0", color: "#555" }}>طباعة</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div>
          ...
          {activeReport !== "flight" && (
            <div style={{ textAlign: "center", padding: "2rem", color: "#aaa" }}>
              ...
              <div style={{ fontSize: 11, marginTop: 4 }}>وزّع الحجاج الأول من صفحات التنظيم</div>
            </div>
          )}
          {activeReport === "flight" && (
            <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
              <button style={{ flex: 1, background: "#1D9E75", color: "white", border: "none", padding: "9px", borderRadius: 8, fontSize: 12, cursor: "pointer", fontWeight: 500 }}>⬇️ تحميل Excel</button>
              <button style={{ flex: 1, background: "transparent", border: "0.5px solid #ddd", padding: "9px", borderRadius: 8, fontSize: 12, cursor: "pointer" }}>📄 تحميل PDF</button>
              <button style={{ flex: 1, background: "transparent", border: "0.5px solid #ddd", padding: "9px", borderRadius: 8, fontSize: 12, cursor: "pointer" }}>🖨️ طباعة</button>
            </div>
          )}
        </div>
      )}
...
export default function App() {
  ...
  const pageTitles: Record<string, string> = { dash: "لوحة التحكم", scan: "رفع وثيقة", passengers: "المسافرون", buses: "الباصات", mina: "مخيمات منى", arafa: "مخيمات عرفة", hotel: "الفندق", reports: "التقارير" };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", direction: "rtl", fontFamily: "system-ui, -apple-system, sans-serif", background: "white", overflow: "hidden" }}>
      <Sidebar page={page} setPage={setPage} count={passengers.length} />
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
