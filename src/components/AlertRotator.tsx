...
function buildAlerts(hajj: Passenger[]): Alert[] {
  ...
  // يوم 1 ذي الحجة 1447 = 2026-05-18
...
function AlertRotator({ passengers, setPage }: { passengers: Passenger[]; setPage: (p: string) => void }) {
  const hajj   = passengers.filter(p => !p.passenger_type || p.passenger_type === "حاج");
  const alerts = buildAlerts(hajj);
  ...
  return (
    <div
      ...
      onClick={() => setPage(cur.page)}
      style={{ borderRadius: 12, border: `1px solid ${cur.borderColor}`, background: cur.bg, cursor: "pointer", overflow: "hidden", position: "relative" }}
      onMouseOver={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 4px 16px ${cur.color}22`; }}
