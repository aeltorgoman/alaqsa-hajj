...
function useSeasonPhases(passengers: Passenger[]) {
  ...
  return useMemo(() => {
    ...
    const fullyDist = hajj.filter(p =>
      p.bus_id != null && p.room_id != null &&
      p.camp_mina_id != null && p.camp_arafa_id != null
    ).length;
    ...
    const phases: PhaseInfo[] = [
      {
        ...
        sub: `${total} حاج · المستندات ${regPct}٪`,
        pct: regPct,
      },
      {
        ...
        sub: distActive ? `اكتمل توزيع ${fullyDist} من ${total} حاج` : `${distributed} من ${PHASE_THRESHOLD} للبدء`,
        pct: distPct,
      },
      {
...
/* ════════════════════════════════════════════════════════════
   كارت مراحل الموسم — قوس متدرج + محطات مدمجة + شريط مهام
   ════════════════════════════════════════════════════════════ */
function SeasonPhaseCard({ passengers, setPage }: { passengers: Passenger[]; setPage?: (p: string) => void }) {
  ...
  /* موقع النقطة على القوس حسب النسبة */
    ...
    <div style={{ background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden", flexShrink: 0, boxShadow: "0 2px 12px rgba(0,0,0,.05)" }}>
      ...
      <div style={{ padding: "15px 18px", display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
        ...
        <div style={{ position: "relative", width: 132, height: 82, flexShrink: 0 }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", paddingBottom: 4 }}>
            {daysToTravel != null && daysToTravel > 0 ? (
              <>
                <div style={{ fontSize: 27, fontWeight: 900, color: primary, lineHeight: 1, fontFamily: "var(--font-heading)" }}>{daysToTravel}</div>
                <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2, fontWeight: 700 }}>يوم على السفر</div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 27, fontWeight: 900, color: primary, lineHeight: 1, fontFamily: "var(--font-heading)" }}>{seasonPct}٪</div>
        ...
        {next && (
          <div style={{ minWidth: 135, background: "var(--ivory)", border: "1px solid var(--line)", borderRadius: 11, padding: "10px 14px" }}>
            <div style={{ fontSize: 10.5, color: accent, marginBottom: 3, fontWeight: 800, filter: "brightness(0.8)" }}>المحطة القادمة</div>
      ...
      <div style={{ borderTop: "1px solid var(--line)", background: "var(--ivory)", padding: "12px 22px 10px" }}>
        <div style={{ position: "relative" }}>
          ...
          <div style={{ position: "absolute", top: 12, right: 28, width: `${(currentIdx / (phases.length - 1)) * 88}%`, height: 3, background: `linear-gradient(to left, ${primary}, ${accent})`, borderRadius: 3 }} />
              ...
              return (
                <div key={ph.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, width: 86 }}>
                  <div style={{
                    ...
                    {done ? (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--ivory)" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
                    ) : (
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke={isCurrent ? "#fff" : "var(--muted)"} strokeWidth="2.2" strokeLinecap="round" dangerouslySetInnerHTML={{ __html: ph.icon }} />
                    )}
                  </div>
                  <span style={{
                    fontSize: 12,
      ...
      <div style={{ borderTop: "1px solid var(--line)", padding: "10px 18px", display: "flex", gap: 20, background: "var(--paper)", flexWrap: "wrap", alignItems: "center" }}>
        {([
          ...
          <span key={item.label}
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2A9D8F" strokeWidth="2" strokeLinecap="round" dangerouslySetInnerHTML={{ __html: item.icon }} />
...
/* ════════════════════════════════════════════════════════════
   كارت توزيع الباقات — ديناميكي حسب باقات الحجاج الفعلية
   ════════════════════════════════════════════════════════════ */

/* ألوان ثابتة لكل باقة معروفة */
...
function PackagesCard({ passengers }: { passengers: Passenger[] }) {
  const hajj = passengers.filter(p => !p.passenger_type || p.passenger_type === "حاج");
  const total = hajj.length;

  /* تجميع ديناميكي حسب hotel_type الفعلي */
  const counts: Record<string, number> = {};
  hajj.forEach(p => {
    const t = p.services?.hotel_type?.trim();
    if (!t) return;
  ...
  return (
    <div style={{ flex: 1, background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 14, padding: "16px 18px", minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <span style={{ fontSize: 14.5, fontWeight: 800, color: "var(--em8)", fontFamily: "var(--font-heading)" }}>توزيع الباقات</span>
...
/* ════════════════════════════════════════════════════════════
   كارت إجمالي الحجاج — للسايدبار (مع رجال · نساء)
   ════════════════════════════════════════════════════════════ */
function TotalPilgrimsCard({ passengers }: { passengers: Passenger[] }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 14, padding: "18px 18px 14px", color: "#fff",
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
        ...
        <div style={{ width: 34, height: 34, borderRadius: 9, background: "rgba(255,255,255,.14)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 10, paddingTop: 9, borderTop: "1px solid rgba(255,255,255,.15)", fontSize: 12.5, fontWeight: 700 }}>
        <span style={{ color: "#9ec3ef" }}>{men} رجال</span>
        <span style={{ opacity: .5 }}>·</span>
        <span style={{ color: "#f2b8cf" }}>{women} نساء</span>
      </div>
    </div>
  );
}
