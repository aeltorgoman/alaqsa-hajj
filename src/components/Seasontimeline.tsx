...
function SmartAlertsCard({ passengers, setPage }: { passengers: Passenger[]; setPage: (p: string) => void }) {
  ...
  const items = useMemo(() => {
    ...
    return [
      ...
      /* ── صلاحية البطاقة — توأم صلاحية الجواز، وبمنطقه: المنتهي
             عطبٌ قائم (‎-1‎) والمقترب عملُ مرحلة التسجيل. ── */
      mk("expired_id",  "بطاقات منتهية الصلاحية", `<rect x="2" y="5" width="20" height="14" rx="2"/><line x1="16" y1="9" x2="8" y2="15"/><line x1="8" y1="9" x2="16" y2="15"/>`, -1, "passengers"),
      mk("expiring_id", "بطاقات تنتهي خلال ٦ أشهر", `<rect x="2" y="5" width="20" height="14" rx="2"/><polyline points="12 9 12 12 14 13.5"/>`, 0, "passengers"),
