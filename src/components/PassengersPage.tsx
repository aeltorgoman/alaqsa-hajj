  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden", fontSize: 13 }}>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        {/* ══ المحتوى الرئيسي ══ */}
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0, marginRight: 280 }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ width: selected ? 0 : 280, flexShrink: 0, display: "flex", flexDirection: "column", background: "var(--paper)", overflow: "hidden", transition: "width .2s ease", position: "fixed", right: 50, top: 46, bottom: 0, zIndex: 20, borderLeft: "1px solid var(--line)" }}>
              {/* هيدر */}
      ...
      {selected && !editing && (
        <div style={{ width: 280, overflowY: "auto", padding: 12, flexShrink: 0, position: "fixed", right: 50, top: 46, bottom: 0, zIndex: 20, borderLeft: "1px solid var(--line)", background: "var(--paper)" }}>
          {(() => {
          ...
          <button onClick={() => {
            /* طباعة الاستيكرات الـ 3 للحاج مباشرة */
            const p = selected;
            const cfg = config as any;
            const primary = cfg.color_primary || "#7D1F3C";
            const accent  = cfg.color_accent  || "#D4A017";
            const cName   = cfg.name_ar || "";
            const season  = cfg.season_label || "";
            const hotel   = cfg.hotel_name || cName;
            const hotelAddr = cfg.hotel_address || "";
            const adminPh = cfg.admin_phone || "";
            const logoHtml = cfg.logo_url
              ? `<img src="${cfg.logo_url}" style="width:62px;height:62px;object-fit:contain;border-radius:50%;border:2.5px solid ${accent};padding:2px;" />`
              : `<div style="width:62px;height:62px;border-radius:50%;border:2.5px solid ${accent};display:flex;align-items:center;justify-content:center;background:#F8F2E4;"><svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="${accent}" stroke-width="1.4"><path d="M12 2l2.4 4.8L19.5 8l-3.5 4 .7 5.5L12 15l-4.7 2.5.7-5.5-3.5-4 5.1-1.2z"/></svg></div>`;
            const shortName = p.short_ar || p.name_ar || "";
            const metaRoom = (metaRooms as any[]).find(r => r.id === (p as any).room_id);
            const metaBus  = (metaBuses as any[]).find(b => b.id === (p as any).bus_id);
            const roomNo   = metaRoom?.number || "—";
            const roomFloor = metaRoom?.floor ? `الدور ${metaRoom.floor}` : "";
            const busName  = metaBus?.name || "";
            const stk = () => `<div style="width:100%;height:99mm;box-sizing:border-box;border-bottom:2px dashed #E8D5C4;display:flex;page-break-inside:avoid;break-inside:avoid;overflow:hidden;position:relative;"><div style="position:absolute;inset:6px;border:2.5px solid ${primary};border-radius:10px;pointer-events:none;"></div><div style="position:absolute;inset:10px;border:1px solid ${accent};border-radius:7px;pointer-events:none;opacity:.55;"></div><div style="width:32%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;padding:10px 8px;border-right:2px solid ${accent};">${logoHtml}<div style="font-size:14pt;font-weight:700;color:${primary};text-align:center;line-height:1.3;font-family:'El Messiri',Cairo,sans-serif;">${cName}</div><div style="font-size:8pt;font-weight:700;color:#8a6a10;text-align:center;">${season}</div><div style="font-size:8.5pt;font-weight:800;color:#241318;direction:ltr;">${adminPh}</div></div><div style="flex:1;display:flex;flex-direction:column;justify-content:center;padding:12px 10px;gap:0;"><div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px dashed #E8D5C4;"><span style="font-size:9pt;font-weight:800;color:#8a6a10;min-width:46px;font-family:Cairo,sans-serif;flex-shrink:0;">الحاج</span><span style="font-size:13pt;font-weight:900;color:#241318;font-family:Cairo,sans-serif;">${shortName}</span></div><div style="display:flex;align-items:center;gap:10px;padding:5px 0;border-bottom:1px dashed #E8D5C4;"><span style="font-size:9pt;font-weight:800;color:#8a6a10;min-width:46px;flex-shrink:0;"></span><span style="font-size:8.5pt;font-weight:600;color:#7A6570;direction:ltr;font-family:Arial,sans-serif;">${p.name_en || ""}</span></div><div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px dashed #E8D5C4;"><span style="font-size:9pt;font-weight:800;color:#8a6a10;min-width:46px;flex-shrink:0;">الفندق</span><span style="font-size:11pt;font-weight:800;color:#241318;font-family:Cairo,sans-serif;">${hotel}</span></div>${hotelAddr ? `<div style="display:flex;align-items:center;gap:10px;padding:5px 0;border-bottom:1px dashed #E8D5C4;"><span style="font-size:9pt;font-weight:800;color:#8a6a10;min-width:46px;flex-shrink:0;">العنوان</span><span style="font-size:9pt;font-weight:700;color:#7A6570;">${hotelAddr}</span></div>` : ""}<div style="display:flex;align-items:center;gap:10px;padding:6px 0;"><span style="font-size:9pt;font-weight:800;color:#8a6a10;min-width:46px;flex-shrink:0;">الهاتف</span><span style="font-size:11pt;font-weight:800;color:#241318;direction:ltr;font-family:Arial,sans-serif;">${p.phone || "—"}</span></div></div><div style="width:26%;margin:10px 10px 10px 0;background:#F8F2E4;border:2px solid ${accent};border-radius:10px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;"><div style="font-size:10pt;font-weight:800;color:#8a6a10;font-family:Cairo,sans-serif;">الغرفة</div><div style="font-size:48pt;font-weight:900;color:${primary};line-height:1;font-family:Cairo,sans-serif;">${roomNo}</div><div style="font-size:8pt;font-weight:800;color:#241318;background:rgba(125,31,60,.1);border-radius:99px;padding:3px 10px;text-align:center;font-family:Cairo,sans-serif;">${roomFloor}${busName ? ` · أوتوبيس ${busName}` : ""}</div></div></div>`;
            const stickerPage = `<div style="width:210mm;height:297mm;background:#fff;display:block;page-break-after:always;font-family:Cairo,sans-serif;direction:rtl;overflow:hidden;">${stk()}${stk()}${stk()}</div>`;
            const html = `<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><link href="https://fonts.googleapis.com/css2?family=Cairo:wght@700;800;900&family=El+Messiri:wght@600;700&display=swap" rel="stylesheet"><style>@page{size:A4;margin:0}body{margin:0;padding:0}*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}</style></head><body>${stickerPage}</body></html>`;
            printInPage(html);
          }} style={{ width: "100%", marginTop: 8, padding: "9px 0", borderRadius: 9, border: "none", background: "var(--paper)", borderTop: "1px dashed var(--line)", color: "var(--ink)", fontFamily: "inherit", fontWeight: 700, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
