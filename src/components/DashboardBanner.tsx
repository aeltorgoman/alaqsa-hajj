...
function DashboardBanner({ passengers, setPage, currentUser }: { passengers: Passenger[]; setPage: (p: string) => void; currentUser: User }) {
  ...
  // ─── عداد يوم عرفة من التقويم الهجري ───
  function getArafaDate(): Date {
    ...
    try {
      ...
      const dateStr = arafaDates[year] || arafaDates[1448];
  ...
  const arafaDate = getArafaDate();
  const now = new Date();
  const diffMs = arafaDate.getTime() - now.getTime();
  const diffDays  = diffMs > 0 ? Math.floor(diffMs / (1000 * 60 * 60 * 24)) : 0;
  const diffHrs   = diffMs > 0 ? Math.floor((diffMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)) : 0;
  const diffMins  = diffMs > 0 ? Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60)) : 0;
    ...
    <div style={{ position: "relative", flexShrink: 0, height: 290 }}>
      ...
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(110deg," + primary + "f0 0%," + primary + " 50%," + primary + "cc 100%)", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(0,0,0,0.18) 0%, transparent 45%)" }} />
        </div>

        {/* ─── شعار الحملة — يمين ─── */}
        <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "40%", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px 24px 50px", zIndex: 4, gap: 14 }}>
        ...
        <div style={{ position: "absolute", top: 14, left: 14, display: "flex", alignItems: "center", gap: 7, zIndex: 5 }}>
          ...
          <div style={{ width: 32, height: 32, borderRadius: 9, background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.18)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
            ...
            <div style={{ position: "absolute", top: 6, left: 6, width: 7, height: 7, borderRadius: "50%", background: "#f87171", border: "1.5px solid rgba(0,0,0,0.35)" }} />
          </div>
        ...
        {/* ─── عداد عرفة — يسار أسفل ─── */}
        <div style={{ position: "absolute", bottom: 50, left: 14, display: "flex", alignItems: "center", gap: 10, background: "rgba(0,0,0,0.38)", border: "1px solid rgba(212,172,79,0.38)", borderRadius: 11, padding: "8px 12px", backdropFilter: "blur(6px)", zIndex: 5 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            {/* أيام */}
            <div style={{ background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 7, padding: "4px 8px", textAlign: "center", minWidth: 44 }}>
              <div style={{ fontFamily: "var(--font-heading)", fontSize: 18, fontWeight: 700, color: "#fbbf24", lineHeight: 1 }}>{diffDays}</div>
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", marginTop: 1 }}>يوم</div>
            </div>
            <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 13, alignSelf: "flex-start", marginTop: 3 }}>:</span>
            {/* ساعات */}
            <div style={{ background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 7, padding: "4px 8px", textAlign: "center", minWidth: 40 }}>
              <div style={{ fontFamily: "var(--font-heading)", fontSize: 18, fontWeight: 700, color: "#e7cd8a", lineHeight: 1 }}>{pad(diffHrs)}</div>
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", marginTop: 1 }}>ساعة</div>
            </div>
            <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 13, alignSelf: "flex-start", marginTop: 3 }}>:</span>
            {/* دقائق */}
            <div style={{ background: "rgba(0,0,0,0.5)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 7, padding: "4px 8px", textAlign: "center", minWidth: 40 }}>
              <div style={{ fontFamily: "var(--font-heading)", fontSize: 18, fontWeight: 700, color: "#e7cd8a", lineHeight: 1 }}>{pad(diffMins)}</div>
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.5)", marginTop: 1 }}>دقيقة</div>
            </div>
          </div>
        </div>
      ...
      {/* ─── كروت الإحصاء المتداخلة أسفل البانر ─── */}
