...
/* مفرداتُ الغرف وسعتُها انتقلت إلى `utils/room.ts` — وحدةٌ بلا
   تبعيّات ليقرأها `src/print` بلا دورةِ استيراد. وتُعاد هنا كما
   كانت، فلا يتغيّر نداءٌ قائم. */
export { ROOM_TYPE_CAP, HOTEL_ROOM_TYPES, isFixedCapType, roomCapacity } from "./room";
...
export const DOC_TTL = {
  ...
  /** بوابة الحاج */
  portal: 15 * 60,
  /* ولا مدخلَ لواتساب: الروابطُ المرسَلة عبره يوقّعها الخادم في
     `whatsapp-send` بمدّته هو، ولا يوقّع المتصفّح شيئاً منها. */
} as const;
