...
export function normalizeCompanyProfile(config: AppConfig | ConfigRow, rows: AssetRow[] = []): CompanyProfile {
  ...
  const assets = Object.fromEntries(rows.map((row): [string, CompanyAsset] => [row.asset_key, {
    key: row.asset_key, url: row.asset_url, altText: row.alt_text,
    metadata: row.metadata ?? ({} as Json), updatedAt: row.updated_at,
  }]));
  if (raw.logo_url && !assets.logo) assets.logo = { key: "logo", url: raw.logo_url, altText: raw.name_ar, metadata: {}, updatedAt: null };

  return {
    identity: { nameAr: raw.name_ar, nameEn: raw.name_en || "", tagline: raw.tagline || "", logoUrl: assets.logo?.url || raw.logo_url || null, seasonLabel: raw.season_label || "" },
    ...
    branding: { primaryColor: raw.color_primary || "#1D9E75", accentColor: raw.color_accent || "#085041", sidebarColor: raw.color_sidebar || "#f9f9f9", bannerUrl: assets.dashboard_banner?.url || raw.banner_image_url || null, bannerPosition: raw.banner_position || "center", bannerPositionX: raw.banner_position_x || "50" },
    reportBranding: { logoUrl: assets.logo?.url || raw.logo_url || "", companyName: raw.name_ar || "نظام الحج", tagline: raw.tagline || "", primaryColor: raw.color_primary || "#1D9E75", accentColor: raw.color_accent || "#085041", headerUrl: assets.report_header?.url || null, footerText: "" },
    portal: {
